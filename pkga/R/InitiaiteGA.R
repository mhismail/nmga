...
InitiateGA <- function (nmods, nindiv, nslots = nindiv, control = NULL, alltokens = NULL, allmods = NULL, seed = 1){
...
NextGA <- function (alltokens,control = NULL, allmods = NULL){
  ...
  if(currentgen%%5 == 0){ # downhill gen
    ...
    best <- filter(allmods1, Fitness == min(allmods1$Fitness, na.rm = T))
  ...
  if(currentgen%%5 == 1 & currentgen > 5){ # generation after downhill
    ...
    best <- filter(allmods1, Fitness == min(allmods1$Fitness, na.rm = T))$Number[1]
    best_path <- paste0("models/All/mod", best)
